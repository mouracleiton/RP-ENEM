...
class ENEMGameIntegration {
    ...
    async init() {
        if (this.initialized) {
            console.warn('ENEM Game Integration already initialized');
            return;
        }

        this.performance.initTime = Date.now();
        console.log('🚀 Starting ENEM RP Game initialization...');
        ...
        try {
            ...
            console.log(`✅ ENEM RP Game initialized successfully in ${this.performance.totalInitTime}ms`);

        } catch (error) {
            console.error('❌ Failed to initialize ENEM RP Game:', error);
    ...
    async initializeSystem(systemName) {
        ...
        try {
            switch (systemName) {
                case 'pluginSystem':
                    if (window.ENEMPluginSystem) {
                        this.systems.pluginSystem = new window.ENEMPluginSystem(this.config.pluginSystem);
                    }
                    break;

                case 'localization':
                    if (window.ENEMLocalization) {
                        this.systems.localization = new window.ENEMLocalization(this.config.localization);
                    }
                    break;

                case 'api':
                    if (window.ENEMGameAPI) {
                        this.systems.api = new window.ENEMGameAPI(this.config.api);
                    }
                    break;

                case 'theme':
                    if (window.ENEMGameTheme) {
                        this.systems.theme = new window.ENEMGameTheme(this.config.theme);
                    }
                    break;

                case 'saveSystem':
                    if (window.ENEMSaveSystem) {
                        this.systems.saveSystem = new window.ENEMSaveSystem(this.config.saveSystem);
                    }
                    break;

                case 'adminPanel':
                    if (window.ENEMAdminPanel) {
                        this.systems.adminPanel = new window.ENEMAdminPanel(this.config.adminPanel);
                    }
                    break;

                case 'analytics':
                    if (window.ENEMAnalyticsSystem) {
                        this.systems.analytics = new window.ENEMAnalyticsSystem(this.config.analytics);
                    }
                    break;

                case 'testSuite':
                    if (window.ENEMTestSuite && this.config.testSuite.enable) {
                        this.systems.testSuite = new window.ENEMTestSuite(this.config.testSuite);
                    }
                    break;
    ...
    // Reiniciar sistema
    async restart() {
        console.log('Restarting ENEM RP Game...');
    ...
    // Destruir sistema
    destroy() {
        console.log('Shutting down ENEM RP Game...');
        ...
        console.log('✅ ENEM RP Game shutdown complete');
    }
...
window.ENEM = window.itaGame;

// Exportar para uso em outros módulos
if (typeof module !== 'undefined' && module.exports) {
    module.exports = ENEMGameIntegration;
}
